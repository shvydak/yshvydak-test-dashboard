...
export class AttachmentManager {
    ...
    private generateFileName(type: AttachmentType, originalFileName?: string): string {
    ...
    async copyPlaywrightAttachment(
        ...
        const fileName =
            originalFileName || this.generateFileName(type, path.basename(sourceFilePath))
    ...
    async deleteAttachment(testResultId: string, fileName: string): Promise<boolean> {
        const filePath = this.getAttachmentPath(testResultId, fileName)
            ...
            if (dirStats.isDirectory()) {
                ...
                for (const file of files) {
                    ...
                    if (fileStats.isFile()) {
                        ...
                        if (['.mp4', '.webm', '.avi'].includes(ext)) type = 'video'
    ...
    async cleanup(daysToKeep: number = 30): Promise<{deletedFiles: number; freedSpace: number}> {
        ...
        for (const testDir of testDirs) {
            ...
            if (dirStats.isDirectory() && dirStats.mtime.getTime() < cutoffTime) {
                const files = await fs.promises.readdir(testDirPath)

...
export default function TestsList({
     ...
     const {tests, error, runTestsGroup, runningGroups, runningTests, getIsAnyTestRunning} = useTestsStore()
     ...
     const groupStats = Object.entries(groupedTests)
          ...
          .map(([filePath, testsInFile]) => {
               ...
               return {
                    ...
                    passed: testsInFile.filter((t) => t.status === 'passed')
                         .length,
                    failed: testsInFile.filter((t) => t.status === 'failed')
                         .length,
                                             ...
                                             <div className="flex items-center space-x-4 text-sm">
                                                  <button
                                                       className={`px-3 py-1 text-xs rounded transition-colors ${
                                                            runningGroups.has(group.filePath) || isAnyTestRunning
                                                       ...
                                                       {runningGroups.has(group.filePath) ? (
                                                                 ...
                                                                 <tr
                                                                      ...
                                                                      <td className="py-3 px-6">
                                                                           <button
                                                                                ...
                                                                                disabled={runningTests.has(test.id) || isAnyTestRunning}
                                                                                className={`text-sm px-3 py-1 rounded-md transition-colors ${
                                                                                     runningTests.has(test.id) || isAnyTestRunning

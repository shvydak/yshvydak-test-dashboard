...
- **Filter-bar counts come from `getTestStatusCounts` (DB-aggregated, unlimited), never from the paginated list.** `testsStore.tests` is capped at 200 without a project tab and 5000 with one. Deriving All/Passed/Failed from it silently caps the badge at page size. Use `useTestStatusCounts`. Symptom: an "All 200" badge that never grows. Without a project, `getTestStatusCounts` excludes tests whose latest row has `project=''` (same filter as the tab badges, `getProjectStatusSummary`); `GET /tests` without a project still returns them.
